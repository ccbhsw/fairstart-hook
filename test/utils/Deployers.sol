...
abstract contract Deployers {
    ...
    function deployPoolManager() internal virtual {
        ...
        } else if (block.chainid == XLAYER_CHAIN_ID) {
            poolManager = IPoolManager(XLAYER_POOL_MANAGER);
        } else {
    ...
    function deployPositionManager() internal virtual {
        if (block.chainid == 31337) {
            ...
            positionManager = IPositionManager(XLAYER_POSITION_MANAGER);
        } else {
    ...
    function deployRouter() internal virtual {
        if (block.chainid == 31337) {
            ...
            // Hook deployment and liquidity scripts do not need the Hookmate router on X Layer.
